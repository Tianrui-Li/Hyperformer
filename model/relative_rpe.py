...
class DynamicRelativePositionBias(nn.Module):
    ...
    def __init__(
        ...
        self.relative_coords_table = self.create_relative_coords_table()
...
class HopRelativePositionBias(nn.Module):
    def __init__(self, num_points, A, num_heads=6, mlp_dim=512, num_frames=64):
        ...
        self.hops = torch.tensor(self.create_hops()).long()
    ...
    def create_hops(self):
        ...
        h1 = self.A.sum(0)
        h1[h1 != 0] = 1
        h = [None for _ in range(self.num_points)]
        h[0] = np.eye(self.num_points)
        h[1] = h1
        hops = 0 * h[0]
